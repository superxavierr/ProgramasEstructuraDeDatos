...
Fecha de asignación: 03/noviembre/2021
Fecha de entrega: 12/noviembre/2021
Programa: Kiosko informativo digital.
 */
...
public class KioskoDigital {
    ...
    public static void main(String[] args) {
        try {
            RandomAccessFile usuarios = new RandomAccessFile("users.txt", "r");
            KeyboardInput teclado = new KeyboardInput();

            int opcion = 0;
            int contadorOportunidades = 1;
            String linea = "";
            String usuarioTeclado = "";
            ...
            while ((contadorOportunidades <= 3) && (opcion != 3)) {
                System.out.println("Introduce el usuario: ");
                usuarioTeclado = teclado.readString();
                System.out.println("Introduce la contraseña: ");
                passwordTeclado = teclado.readString();
                System.out.println("MD5: " + KioskoDigital.MD5(passwordTeclado));
                linea = "";
                usuarios.seek(0);
                Epuntero = usuarios.getFilePointer();
                finArchivo = usuarios.length();
                while ((Epuntero < finArchivo) && acceso == false) {
                    linea = usuarios.readLine();
                    System.out.println(linea);
                    usuarioArchivo = "";
                    passwordArchivo = "";
                    String arrayUser[] = linea.split("\\|");
                    usuarioArchivo = arrayUser[0];
                    passwordArchivo = arrayUser[1];
                    System.out.println("Renglon de usuario: " + usuarioArchivo);
                    System.out.println("Renglón de contraseña: " + passwordArchivo);
                    if ((usuarioTeclado.compareTo(usuarioArchivo) == 0) && (passwordTeclado.compareTo(passwordArchivo) == 0)) {
                        acceso = true;
                ...
                if (acceso == true) {
                    System.out.println("USUARIO VALIDO, BIENVENIDO AL SISTEMA");
